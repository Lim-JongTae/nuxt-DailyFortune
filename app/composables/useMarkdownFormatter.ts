...
export const useMarkdownFormatter = () => {
  const formatMarkdown = (rawText: string | undefined | null): string => {
    ...
    text = text.replace(/^[\*\-] (.*$)/gim, `
      <li class="ml-2 my-2 flex items-start gap-2 text-[#1f2937] dark:text-[#f8fafc] text-xs sm:text-sm leading-relaxed font-normal">
    ...
    const formattedHtml = lines.map((line: string) => {
      ...
      return `<p class="text-[#1f2937] dark:text-[#f8fafc] leading-relaxed my-2.5 text-xs sm:text-sm font-normal tracking-normal">${line}</p>`
    }).join('\n')
